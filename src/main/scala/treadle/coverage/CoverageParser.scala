/*
Copyright 2020 Department of Applied Mathematics and Computer Science, Technical University of Denmark
...
object CoverageParser {
  ...
  /**
    ...
    */
  private def getIndexOfInputs(firrtlSourceList: List[String]) : Int =
  ...
  /**
    ...
    */
  private def generateValidatorDef(firrtlSourceList: List[String], neededVals: Int) : List[String] = {
  ...
  /**
    ...
    */
  private def retrieveMuxCondition(muxLine: String): String =
  ...
  /**
    ...
    */
  @tailrec
  private def constructValidatorCode(
      ...
      constructValidatorCode(muxLines.tail, muxConditions.tail, index + 1,
        acc :+ List(muxLines.head,
          muxLineIndent + validatorName  + (2 * index) +
            " <= mux(" + muxConditions.head + ", UInt<1>(1), UInt<1>(0))",
          muxLineIndent + validatorName + (2 * index + 1) +
            " <= mux(" + muxConditions.head + ", UInt<1>(0), UInt<1>(1))"
        )
      )}
  }
  ...
  /**
    ...
    */
  @tailrec
  private def constructSourceWithVals(
  ...
  /**
    ...
    */
  @tailrec
  private def constructNewSource(
